...
public class ReadTicketFlyHTML {
	...
	//private static String[] URLS = {"marquis-theater", "hi-dive", "globe-hall",	"cervantes"};
	private static String[] URLS = {"http://www.ticketfly.com/venue/3699-hi-dive/", "http://www.ticketfly.com/venue/16505-marquis-theater/"};
	private static String[] genres = {"rock", "pop", "rap", "country", "folk", "r&b", "latin", "edm", "experimental", "garage"};
	private static String GetVenueEventListQuery = "";
	private static String GetLastFMArtist = "/2.0/?method=artist.search&artist=";
	private static String LastFMAPIKey = APIKEY
	...
	private ArrayList<Event> takeHTML(Document docArg) {
		...
		try {
			...
			System.out.println("Event Elements found: " + eventElements.size());
			...
			for (Element event : eventElements) {
				...
				try {
					...
					System.out.println("Individual event acquisition failed: " + iae);
				}
			...
			System.out.println("Venue event acquisition failed: " + e);
			myLogger.addErrorMessage("There was an issue in the takeHTML method when attempting to get the list of event elements.");
		}
		return eventObjects;
	}
	
	private Event createEvent(Element eventArg) throws IllegalArgumentException {
		...
		if (pageSpecific.getElementById("when") != null) {
			//currentEvent.setEventDateAndTime();
			String htmlDate = pageSpecific.getElementById("when").getElementsByTag("meta").first().attr("content") + " " + pageSpecific.getElementsByClass("time").first().html();
			htmlDate = htmlDate.split(" ")[0];
			System.out.println(htmlDate);
			Date eventDate = java.sql.Date.valueOf(htmlDate);
			currentEvent.setEventDateAndTime(eventDate);
		}
